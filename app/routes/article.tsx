...
export default function Article() {
  ...
  const { data: articleData, isLoading } = useQuery({
    queryKey: ["articleData", paper_id],
    queryFn: () => (paper_id ? getPostById(paper_id) : Promise.resolve(null)),
  });
  const { data: content, isLoading: contentLoading } = useQuery({
    queryKey: ["articleContent", paper_id],
    queryFn: () =>
      paper_id ? getPostMarkdownById(paper_id) : Promise.resolve(null),
  });
  const { data: reactionData } = useQuery({
    queryKey: ["reactionData", paper_id],
    queryFn: () => (paper_id ? getReaction(paper_id) : Promise.resolve(null)),
  });
  const { data: commentData } = useQuery({
    queryKey: ["commentData", paper_id],
    queryFn: () =>
      paper_id ? getPostCommentsById(paper_id) : Promise.resolve(null),
  });
    ...
    onMutate: (reaction) => {
      ...
      queryClient.setQueryData(
        ...
        (old: ArticleData | undefined) => {
          ...
          if (reaction === "like") {
            updatedData.like_count +=
              reactionData === reactionType.like ? -1 : 1;
            if (reactionData === reactionType.dislike)
              updatedData.dislike_count -= 1;
            queryClient.setQueryData(
              ["reactionData", paper_id],
              reactionType.like,
            );
          } else if (reaction === "dislike") {
            updatedData.dislike_count +=
              reactionData === reactionType.dislike ? -1 : 1;
  ...
  const handlePostCommentMutation = useMutation({
    mutationFn: (content: string) => {
      if (!paper_id)
        throw new Error("Paper ID is required for posting comment");
    ...
    onMutate: async (newCommentContent) => {
      ...
      const previousComments = queryClient.getQueryData<CommentItemProps[]>([
        "commentData",
        paper_id,
      ]);
      const newComment: CommentItemProps = {
        ...
        profile:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQqXtvUw93BzewwknzouqY0JtoKUPNBDcXbuw&s",
        name: "asdf",
        time: "방금 전",
      ...
      queryClient.setQueryData(
        ["commentData", paper_id],
        (old: CommentItemProps[] | undefined) => {
  ...
  const handleBookmarkMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      queryClient.setQueryData(["userData"], (old: any) => {
        ...
        return {
          ...old,
          bookmarks: updateBookmarkState(old.bookmarks, paper_id!),
        };
      });
          ...
          <FeedbackBox>
            <ButtonContainer>
              <FeedbackButtonBox>
                ...
                <GoodButton
                  $fill={reactionData ? "#F7971D" : "#D9D9D9"}
                  onClick={async () => await handleClickReaction("like")}
                />
              ...
              <FeedbackButtonBox>
                ...
                <BadButton
                  $fill={reactionData === false ? "#F7971D" : "#D9D9D9"}
                  onClick={async () => await handleClickReaction("dislike")}
                />
